...
def contacts(trajectory, cutoff: float = 0.35, countatoms: bool = True):
    ...
    contact_df = pd.DataFrame(columns=['Frame', 
                                       'chainA_atomID', 
                                       'chainA_name', 
                                       'chainA_resn', 
                                       'chainA_resi', 
    ...
    for framenum, frame in enumerate(trajectory):
        frame_contacts = contact_df.iloc[0:0].copy()
        ...
        for index, d in enumerate(distances[framenum,]):
            if d < cutoff:
                ...
                if not countatoms: 
                    frame_contacts = pd.DataFrame([{'Frame':framenum, 
                                                    'chainA_atomID': pd.NA,  
                                                    'chainA_name': pd.NA, 
                            ...
                            if i != atom: 
                                ...
                                frame_contacts = pd.concat([frame_contacts if not frame_contacts.empty else None, 
                                                            pd.DataFrame([{'Frame':framenum, 
                                                                           ...
                                                                           'distance': d}])], 
                                                           ignore_index=True)
                contact_df = pd.concat([contact_df if not contact_df.empty else None,
...
def main():
    ...
    df = contacts(data.input_traj, 0.35)
